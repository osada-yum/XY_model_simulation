...
progfile="./app/xy3d_periodic_wolff_equilibrium.f90"
execfile="${root_dir}/bin/xy3d_periodic_wolff_equilibrium"
outputdir="data/3D-XY_equilibrium"
outputfile="${outputdir}/3D-XY_Wolff_x${nx}_y${ny}_z${nz}_discard${mcs_discard}_sample${mcs_sample}_kbt${kbt_begin}-${kbt_end}_n${kbt_nsplit}_iseed${iseed}_$(date +%Y%m%d_%H%M%S).dat"
mkdir -p ${outputdir}
tmpdir="data/tmp"
mkdir -p "${tmpdir}"
...
echo "3D-XY_Wolff,  ${nx}x${ny}x${nz} == $((nx*ny*nz)), discard ${mcs_discard}, sample ${mcs_sample}, T: (${kbt_begin}-${kbt_end}_${kbt_nsplit}), iseed ${iseed}, time ${elapsed_time}, ${outputfile}" | tee -a xy3d_equilibrium.log
