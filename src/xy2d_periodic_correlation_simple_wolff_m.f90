module xy2d_periodic_correlation_simple_wolff_m
...
contains
  ...
  !> calc_magne_abs: Calculate the absolute value of magnetization for the lattice.
  pure real(real64) function calc_magne_abs() result(res)
    integer(int64) :: x, y
    real(real64) :: res_x, res_y
    res_x = 0.0d0
    res_y = 0.0d0
    do y = 1, ny
       do x = 1, nx
          res_x = res_x + spins(1, x, y)
          res_y = res_y + spins(2, x, y)
       end do
    end do
    res_x = res_x * nall_inv
    res_y = res_y * nall_inv
    res = hypot(res_x, res_y)
...
end module xy2d_periodic_correlation_simple_wolff_m
