module xy2d_periodic_correlation_simple_m
  ...
  integer(int64), parameter, public :: nx = 100, ny = nx, nall = nx * ny
  ...
  contains
    pure subroutine local_overrelaxation(spins, x, y)
      ...
      block
        ...
        spins(1:2, x, y) = &
             & (2 * sum(local_field(1:2) * spins(1:2, x, y)) * abs_local_field_inv**2) * local_field(1:2) - spins(1:2, x, y)
      end block
...
program xy2d_periodic_metropolis_equilibrium_simulation
  ...
  integer(int32), parameter :: overrelaxation_per_mcs = 0
