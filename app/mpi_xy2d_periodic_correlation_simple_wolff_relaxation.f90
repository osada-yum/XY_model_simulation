program mpi_xy2d_periodic_correlation_simple_wolff_simulation
  ...
  order_parameter(:) = variance_covariance_kahan()
  ...
  if (myrank == 0) then
     ...
     write(output_unit, '(a)' ) "# method: Wolff, model: 2D-XY"
     ...
     write(error_unit, '(a)' ) "# method: Wolff, model: 2D-XY"
    ...
    if (myrank == 0) then
       ...
       do i = 1, mcs
          write(output_unit, '(*(g0, 1x))', advance = "no") nall, nsample * num_proc, i, &
               ...
               & all_order_params(i)%var1(), all_order_params(i)%var2(), all_order_params(i)%cov()
          do j = 1, n_unique_correlation
...
end program mpi_xy2d_periodic_correlation_simple_wolff_simulation
