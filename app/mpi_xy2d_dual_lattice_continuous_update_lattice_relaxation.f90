program mpi_xy2d_dual_lattice_continuous_update_simulation
  use, intrinsic :: iso_fortran_env
  use mpi
  use gf2xe
  use msmt19937
  use xy2d_dual_lattice_continuous_update_m
  ...
  if (myrank == 0) then
     ...
     write(output_unit, '(a)' ) "# continuous update lattice"
     ...
     write(error_unit, '(a)' ) "# continuous update lattice"
  end if
  do j = 1, nsample
     if (myrank == 0) &
     ...
     do i = 1, mcs
        call update_metropolis_continuous_lattice()
...
end program mpi_xy2d_dual_lattice_continuous_update_simulation
